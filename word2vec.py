...
def CBOW(centerWord, contextWords, inputMatrix, outputMatrix):
    ...
    loss = 0
    for prob in softmax:
        loss -= torch.log(prob[centerWord])
    ...
    for _ in range(epoch):
        ...
        for center, contexts in train_seq:
            ...
            if mode == "CBOW":
                ...
                L, G_in, G_out = skipgram(center, contexts, W_in, W_out)

                W_in[center] -= learning_rate * G_in.squeeze()
                W_out -= learning_rate * G_out
...
def main():
    ...
    # Training section
    emb, _ = word2vec_trainer(train_set, len(w2i), freqtable, mode=mode, dimension=64, epoch=1, learning_rate=0.001)
